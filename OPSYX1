--  SUITE V9.45.3-DEFENSIVE-STABLE - OPSYX  (FULL AUDIT / UI SAFE / VERTICAL ADVANCED SUITE)
...
\n-- ============================================================\n-- OPSYX LICENSE / RAW-THEFT PROTECTION GATE\n-- ============================================================\n-- This raw file is not the authority by itself. It requires the OPSYX key\n-- system to establish a short-lived launch context, then performs a second\n-- Platoboost whitelist verification for the same Roblox UserId/key pair.\n-- No HWID/device/MAC/IP identifier is used.\n--\n-- IMPORTANT LIMITATION:\n-- A client-side gate can stop unauthorized/casual execution, but it cannot\n-- make public Lua source impossible to copy or modify. For strong IP/source\n-- protection, the actual implementation must be served from a private\n-- authorization endpoint rather than a public GitHub raw URL.\n-- ============================================================\ndo\n    local GATE_PLAYERS = game:GetService("Players")\n    local GATE_HTTP = game:GetService("HttpService")\n    local GATE_PLAYER = GATE_PLAYERS.LocalPlayer or GATE_PLAYERS.PlayerAdded:Wait()\n    local GATE_USER_ID = tostring(GATE_PLAYER.UserId)\n    local GATE_MAX_AGE = 45\n\n    local function gateRequestFunction()\n        local f\n        pcall(function() if type(syn) == "table" and type(syn.request) == "function" then f = syn.request end end)\n        if type(f) == "function" then return f end\n        if type(request) == "function" then return request end\n        if type(http_request) == "function" then return http_request end\n        if type(syn_request) == "function" then return syn_request end\n        pcall(function() if type(http) == "table" and type(http.request) == "function" then f = http.request end end)\n        if type(f) == "function" then return f end\n        pcall(function() if type(fluxus) == "table" and type(fluxus.request) == "function" then f = fluxus.request end end)\n        return f\n    end\n\n    local function gateSHA256(value)\n        local ok, result = pcall(function()\n            if type(crypt) == "table" and type(crypt.hash) == "function" then\n                return crypt.hash(tostring(value), "sha256")\n            end\n            if type(syn) == "table" and type(syn.crypt) == "table" and type(syn.crypt.hash) == "function" then\n                return syn.crypt.hash(tostring(value), "sha256")\n            end\n            return nil\n        end)\n        if ok and type(result) == "string" and #result > 0 then\n            return string.lower(result)\n        end\n        return nil\n    end\n\n    local function gateURL(value)\n        return tostring(value):gsub("[^%w%-%._~]", function(c)\n            return string.format("%%%02X", string.byte(c))\n        end)\n    end\n\n    local ctx = nil\n    pcall(function() ctx = rawget(_G, "__OPSYX_LICENSE_CONTEXT") end)\n\n    if type(ctx) ~= "table" then\n        error("[OPSYX] Unauthorized launch. Start OPSYX through the OPSYX key system.")\n    end\n\n    if tostring(ctx.userId or "") ~= GATE_USER_ID then\n        error("[OPSYX] License UserId mismatch. Access denied.")\n    end\n\n    local gateKey = tostring(ctx.key or "")\n    local gateIssued = tonumber(ctx.issuedAt)\n    local gateNonce = tostring(ctx.nonce or "")\n    local gateProof = tostring(ctx.proof or "")\n\n    if gateKey == "" or gateNonce == "" or not gateIssued then\n        error("[OPSYX] Invalid launch context. Access denied.")\n    end\n\n    if math.abs(os.time() - gateIssued) > GATE_MAX_AGE then\n        error("[OPSYX] Launch authorization expired. Please verify again.")\n    end\n\n    -- The key system creates this proof. It prevents accidental/malformed\n    -- contexts from being accepted by the raw. It is not a substitute for\n    -- server-side authentication because the secret is client-visible.\n    local expectedProof = gateSHA256(\n        GATE_USER_ID .. "|" .. gateKey .. "|" .. gateNonce .. "|" ..\n        "OPSYX-LAUNCH"\n    )\n\n    if not expectedProof or gateProof:lower() ~= expectedProof:lower() then\n        error("[OPSYX] Launch authorization proof failed. Access denied.")\n    end\n\n    local identifier = gateSHA256(GATE_USER_ID)\n    if not identifier then\n        error("[OPSYX] SHA-256 is unavailable. Access denied.")\n    end\n\n    local requester = gateRequestFunction()\n    if type(requester) ~= "function" then\n        error("[OPSYX] HTTP request support is unavailable. Access denied.")\n    end\n\n    local function verifyHost(host)\n        local nonce = ""\n        for i = 1, 16 do\n            nonce = nonce .. string.char(math.random(97, 122))\n        end\n\n        local url = host .. "/public/whitelist/" .. tostring(31267)\n            .. "?identifier=" .. gateURL(identifier)\n            .. "&key=" .. gateURL(gateKey)\n            .. "&nonce=" .. gateURL(nonce)\n\n        local ok, response = pcall(function()\n            return requester({\n                Url = url,\n                Method = "GET",\n                Headers = { ["Accept"] = "application/json" },\n            })\n        end)\n\n        if not ok or type(response) ~= "table" then\n            return false, "NETWORK"\n        end\n\n        local status = tonumber(response.StatusCode or response.status_code)\n        if status ~= 200 then\n            return false, "HTTP_" .. tostring(status or "UNKNOWN")\n        end\n\n        local body = response.Body or response.body\n        if type(body) ~= "string" or body == "" then\n            return false, "MALFORMED"\n        end\n\n        local decodedOK, decoded = pcall(function()\n            return GATE_HTTP:JSONDecode(body)\n        end)\n\n        if not decodedOK or type(decoded) ~= "table" then\n            return false, "MALFORMED"\n        end\n\n        if not (decoded.success == true and type(decoded.data) == "table") then\n            return false, tostring(decoded.reason or decoded.message or "INVALID")\n        end\n\n        if decoded.data.valid ~= true then\n            return false, tostring(decoded.reason or decoded.message or "INVALID")\n        end\n\n        local returnedHash = tostring(decoded.data.hash or "")\n        local expectedHash = gateSHA256("true-" .. nonce .. "-" .. tostring(ctx.secret or ""))\n\n        if expectedHash and returnedHash:lower() == expectedHash:lower() then\n            return true, "SUCCESS"\n        end\n\n        return false, "INTEGRITY"\n    end\n\n    local checked = false\n    local lastReason = "NETWORK"\n\n    -- Keep the Platoboost secret out of this raw file's API-hash check if the\n    -- key system supplied a precomputed response proof. If absent, fail closed.\n    if tostring(ctx.secret or "") == "" then\n        error("[OPSYX] License verification context is incomplete. Access denied.")\n    end\n\n    for _, host in ipairs({\n        "https://api.platoboost.app",\n        "https://api.platoboost.net",\n    }) do\n        local ok, reason = verifyHost(host)\n        if ok then\n            checked = true\n            break\n        end\n        lastReason = reason\n    end\n\n    if not checked then\n        error("[OPSYX] License re-verification failed: " .. tostring(lastReason))\n    end\n\n    print("[OPSYX] License gate passed for UserId " .. GATE_USER_ID)\nend\n\n-- ============================================================
-- RE-EXECUTION GUARD
...
local ESP_LABEL_GAP_PX  = 32
local ESP_LABEL_GAP_X   = 230
...
-- too weak or too strong.  Capture the sensitivity at script start as the
-- reference and compensate subsequent changes relative to that baseline.
local UserGameSettings = nil
local AIM_BASE_SENS    = 1
local AIM_CURRENT_SENS = 1
...
local function refreshAimSensitivity()
    ...
    if not sens or sens <= 0 or sens ~= sens then
        -- Unreadable/absent: keep the last known-good mapping. Never
        -- snap the scale back to 1 mid-aim (that re-introduces the
        -- original sensitivity-dependent jump).
        return
    end
    if not sensBootstrapped then
        -- First successful read defines the reference feel: the
        -- sensitivity active when the script was injected.
        AIM_BASE_SENS    = sens
    ...
    AIM_CURRENT_SENS = sens
    -- Roblox camera rotation ~= mouse delta * MouseSensitivity.
    -- Scaling output by BASE/CURRENT (raising sensitivity -> smaller
    -- delta, lowering -> larger delta) keeps the produced rotation
    -- identical at any sensitivity. Bounds cover the full settings
    -- slider (0.1..10) plus programmatic values; wider than the
    -- original 0.25..4.0 clamp.
    AIM_SENS_SCALE = cl(AIM_BASE_SENS / AIM_CURRENT_SENS, 0.05, 20.0)
end
...
local function toggleMasterUIVisibility()
    ...
    local show = not MASTER_UI_HIDDEN
    GUI.main.Visible = show
    ST.hid = not show
    if not show then
        ST.__closeAuxPanels("none")
        if GUI.mobilePanel then GUI.mobilePanel.Visible = false end
        ...
        if GUI.mobilePanel then GUI.mobilePanel.Visible = MOB end
        if GUI.restoreBar then GUI.restoreBar.Visible = false end
        if GUI.advancedSuite and S.V40 and S.V40.suiteVisible == true then
...
local function doAimbot(dt)
    ...
    -- Compensate relative mouse output for Roblox sensitivity changes while
    -- preserving the original behavior at the sensitivity used at startup.
...
local function cg()
    ...
    makeToggle("HOLD AIM", CARD_X + 6*(CARD_W+CARD_GAP), CARD_Y, CARD_W, C.C_ORG, C.C_GRN,
        ...
        function()
            ...
            if not holdToAimEnabled then
                ...
                ST.htArm = false
                ST.holdReleased = false
                if ST.tgpl ~= nil then C.flushTarget() end
            else
                C.clearHeadCache()
            end
        end)
    ...
    local ACTION_Y, ACTION_W, ACTION_GAP = 109, 138, 4
    ...
    local function fcApplyConfig(d)
        ...
        cp(C.S.KB, d.KB); cp(C.S.AM,d.AM); cp(C.S.SL,d.SL); cp(C.S.TR,d.TR); cp(C.S.ES,d.ES)
    ...
    hook(UI.InputBegan:Connect(function(inp, gpd)
        ...
        if ST._rb then
            ...
            if inp.KeyCode == Enum.KeyCode.Escape then
                nn = nil
                pcall(function() rb.btn.Text = rb.old or "NONE"; rb.btn.TextColor3 = UI_ACTIVE end)
                ST._rb = nil
                return
            ...
            elseif inp.UserInputType == Enum.UserInputType.MouseButton1
                ...
                or inp.UserInputType == Enum.UserInputType.MouseButton4
                or inp.UserInputType == Enum.UserInputType.MouseButton5 then
                nn = EN[inp.UserInputType] or tostring(inp.UserInputType):gsub("Enum.UserInputType.","")
            elseif inp.UserInputType ~= Enum.UserInputType.None
                and inp.UserInputType ~= Enum.UserInputType.MouseMovement
                and inp.UserInputType ~= Enum.UserInputType.MouseWheel
        ...
        -- Configurable global controls are handled before GameProcessed so
        -- the user-selected keys remain reliable even while Roblox is processing input.
        if mk(inp, "hide") then
            if not MASTER_UI_HIDDEN then toggleMenuVisibility() end
            return
        end
        ...
        if mk(inp, "hold") then
            holdToAimEnabled = not holdToAimEnabled
            S.AM.holdMode = holdToAimEnabled
            S.AM.activationMode = holdToAimEnabled and "HOLD" or "TOGGLE"
            if not holdToAimEnabled then
                S.AM.on = false
                aiming = false
                ST.arm = false; ST.saArm = false
        ...
        if mk(inp,"am") then
            if S.AM.activationMode ~= "HOLD" and not holdToAimEnabled then
                S.AM.on = not S.AM.on
                if not S.AM.on and ST.tgpl ~= nil then flushTarget() end
                updBtn("AIMBOT", S.AM.on, C_GRN, C_RED)
            end
        end
        if mk(inp,"es") then
...
if MOB then
    pcall(function()
        ...
        mToggle("SILENT", C_ORG, C_GRN,
            ...
            function() S.SL.on=not S.SL.on; if S.SL.on then ST.saArm=true end end)

        mToggle("TRIGGER", C_RED, C_GRN,
            function() return S.TR.on end,
            function() S.TR.on=not S.TR.on; ST.mobArm=S.TR.on end)
...
ST.v39.okLoop, ST.v39.errLoop = pcall(function()
    ST.v39.loopConn = RS.RenderStepped:Connect(function(dt)
        ...
        pcall(function()
            ...
            if not hit then
                pcall(function()
                    for _, child in ipairs(RPS:GetChildren()) do
    ...
    local V40_OK, V40_ERR = pcall(function()
        ...
        if savedSuiteSize and savedSuiteSize.resized and savedSuiteSize.w and savedSuiteSize.h then
            suite.Size = UDim2.fromOffset(
                cl(tonumber(savedSuiteSize.w) or SUITE_W, 660, 900),
                cl(tonumber(savedSuiteSize.h) or initialH, 440, 620)
            )
        else
        ...
        local function makeAdvButton(section, key, textValue, callback, opts)
            ...
            conn = b.Activated:Connect(function()
                ...
                pcall(function()
                    if ADV.refresh then ADV.refresh() end
                end)
            end)
        ...
        local function setAdvButton(b, textValue, enabled)
            ...
            elseif enabled then
                ...
                b.BackgroundColor3 = UI_PANEL_INPUT
                b.TextColor3 = UI_TEXT_SECONDARY
            end
        end
        ...
        makeAdvButton(aimCard, "aim_enable", "", function()
            if S.AM.activationMode == "HOLD" or holdToAimEnabled then
                notify("Aim mode is HOLD; use RMB to activate")
                return
            end
            S.AM.on = not S.AM.on
            if not S.AM.on then flushTarget() end
        end, {boolKey="aimEnable", order=10})

        makeAdvButton(aimCard, "aim_part", "", function()
            local opts = {"Head","UpperTorso","HumanoidRootPart","Torso","LowerTorso","Auto"}
            local nextPart = cycleBound(S.AM.targetPart, opts)
            S.AM.targetPart = nextPart
            STATE.targetPart = nextPart
            clearHeadCache()
        end, {order=20})
        ...
        makeAdvButton(aimCard, "aim_team", "", function()
            S.AM.tc = not S.AM.tc
        end, {boolKey="aimTeam", order=40})

        makeAdvButton(aimCard, "aim_visibility", "", function()
            S.AM.wc = not S.AM.wc
        end, {boolKey="aimVisibility", order=50})

        makeAdvButton(aimCard, "aim_alive", "", function()
            S.AM.aliveCheck = not S.AM.aliveCheck
        end, {boolKey="aimAlive", order=60})
        ...
        makeAdvButton(aimCard, "aim_lock", "", function()
            S.AM.targetLock = not S.AM.targetLock
            S.AM.sticky = S.AM.targetLock
            STATE.sticky = S.AM.targetLock
            if not S.AM.targetLock then
                ST.tgpl = nil
            end
        end, {boolKey="targetLock", order=90})

        makeAdvButton(aimCard, "aim_switch", "", function()
            S.AM.targetSwitching = not S.AM.targetSwitching
        end, {boolKey="targetSwitching", order=100})
        ...
        makeAdvButton(crossCard, "cross_enable", "", function()
            STATE.crosshair = not STATE.crosshair
            if not STATE.crosshair and destroyCrosshair then pcall(destroyCrosshair) end
        end, {boolKey="crosshair", order=10})
        ...
        makeAdvButton(crossCard, "cross_dot", "", function()
            STATE.crosshairDot = not STATE.crosshairDot
        end, {boolKey="crosshairDot", order=50})

        makeAdvButton(crossCard, "cross_outline", "", function()
            STATE.crosshairOutline = not STATE.crosshairOutline
        end, {boolKey="crosshairOutline", order=60})
        ...
        makeAdvButton(espCard, "esp_enable", "", function()
            S.ES.on = not S.ES.on
            if not S.ES.on then
                destroyAllInstanceESP()
                if extraClearAll then extraClearAll() end
            end
        end, {boolKey="espEnable", order=10})
        ...
        makeAdvButton(espCard, "esp_name", "", function()
            S.ES.name = not S.ES.name
        end, {boolKey="espName", order=30})

        makeAdvButton(espCard, "esp_distance", "", function()
            S.ES.distance = not S.ES.distance
        end, {boolKey="espDistance", order=40})

        makeAdvButton(espCard, "esp_health", "", function()
            S.ES.health = not S.ES.health
        end, {boolKey="espHealth", order=50})

        makeAdvButton(espCard, "esp_tracer", "", function()
            S.ES.tracer = not S.ES.tracer
        end, {boolKey="espTracer", order=60})

        makeAdvButton(espCard, "esp_highlight", "", function()
            S.ES.highlight = not S.ES.highlight
        end, {boolKey="espHighlight", order=70})

        makeAdvButton(espCard, "esp_team", "", function()
            S.ES.tc = not S.ES.tc
            pcall(refreshAllESPFilterState)
        end, {boolKey="espTeam", order=80})

        makeAdvButton(espCard, "esp_visibility", "", function()
            S.ES.visibility = not S.ES.visibility
        end, {boolKey="espVisibility", order=90})
        ...
        makeAdvButton(perfCard, "perf_lightweight", "", function()
            STATE.lightweight = not STATE.lightweight
            if STATE.lightweight then
                S.ES.updateRate = cl(tonumber(S.ES.updateRate) or 10, 3, 15)
            end
        end, {boolKey="lightweight", order=20})
        ...
        makeAdvButton(themeCard, "theme_compact", "", function()
            STATE.compactMode = not STATE.compactMode
            refreshAdvVisuals()
        end, {boolKey="compactMode", order=20})
        ...
        makeAdvButton(keyCard, "key_reset", "RESET KEYBINDS", function()
            ...
            if ST._rb then ST._rb = nil end
        end, {order=120})

        -- SAFETY QUICK ACTION ----------------------------------------
        local safetyCard = makeAdvSection(
            "08 - SAFETY QUICK ACTION", 8,
        ...
        makeAdvButton(safetyCard, "safety_enable_relevant", "ENABLE RELEVANT", function()
            ...
            S.AM.on = S.AM.activationMode ~= "HOLD" and true or false
            S.SL.on = true
            S.TR.on = true
            S.ES.on = true
            S.FV.on = true
            STATE.crosshair = true
            holdToAimEnabled = S.AM.activationMode == "HOLD"
            if not holdToAimEnabled then
                ST.holdReleased = false
            end
        ...
        makeAdvButton(safetyCard, "safety_pause_loop", "", function()
            STATE.runtimePaused = not STATE.runtimePaused
        end, {boolKey="runtimePaused", order=60})
        ...
        local diagCard = makeAdvSection(
            "10 - OPSYX PROTECTION DIAGNOSTICS", 10,
        ...
        applyLayout = function(name)
            ...
            local suiteWidths = {COMPACT=680, STANDARD=760, WIDE=840}
        ...
        local function ensureGuiCrosshair()
            if not crossGui then return false end
        ...
        importClipboard = function()
            ...
            if type(data.KB) == "table" then for k,v in pairs(data.KB) do if S.KB[k] ~= nil and type(v) == "string" then S.KB[k]=v end end end
...
pcall(function()
    ...
    makeResizable(GUI.advancedSuite, "advancedSuite", 260, 320, 900, 620)
